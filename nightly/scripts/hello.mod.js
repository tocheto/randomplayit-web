...
hello.utils.extend( hello, {
	...
	login :  function(){
		...
		p.qs = {
			...
			display		: opts.display,	//v100++ san - Dropbox complains for unknown parameter
			scope		: 'basic',
			state		: {
				client_id	: provider.id,
				network		: p.network,
...
hello.init({
	'dropbox' : {
		...
		*/
		...
		// Map GET requests
		get : {
			"audio"		: 'search/auto/?query=.mp3',/*req("metadata/@{root|dropbox}/?query=.mp3"),*/
...
*/
	...
	hello.init({
		google : {
			...
			scope : {
				basic 			: 'https://www.googleapis.com/auth/drive.readonly',//"https://www.googleapis.com/auth/plus.me profile",
			...
			// Map GET requests
			get : {
				'audio' : "drive/v2/files?q=mimeType contains 'audio/'&fields=items(title,id)"	
			},
...
hello.init({
	windows : {
		...
		// Authorization scopes
		scope : {
			basic			: 'wl.skydrive,wl.signin'//'wl.signin,wl.basic',
		},
		...
		// Map GET requests
		get : {
			"audio" : '@{parent|me/skydrive}/files?filter=audio,folders',
			"link" : '@{parent|me/skydrive}/files?filter=audio,folders'
		},
...
//
// 4shared
// hhttp://www.4shared.com/developer/docs/
...
hello.init({
	'4shared' : {
		...
		oauth : {
			version : 1,
			auth	: "https://api.4shared.com/v0/oauth/authorize",
			request : 'https://api.4shared.com/v0/oauth/initiate',
			token	: 'https://api.4shared.com/v0/oauth/token'
		},

		// Request path translated
		base : 'https://api.4shared.com/v0/',
		get : {
			'me' : 'user.json',
			'audio' : 'user/files.jsonp?category=1'
			}
		},
		// Response handlers
		wrap : {
...
function formatUser(o){
	if(o.id){
		var token = hello.getAuthResponse('spotify').access_token;
		if(o.emails){
			o.email =  o.emails.preferred;
		}
		// If this is not an non-network friend
...
hello.init({
	spotify : {
		...
		// Authorization scopes
		scope : {
			basic			: 'playlist-read-private user-library-read'//'wl.signin,wl.basic',
		},
...
hello.init({
	beats : {
		...
		// REF: https://developer.beatsmusic.com/io-docs
		oauth : {
			version : 2,
			auth : 'https://partner.api.beatsmusic.com/v1/oauth2/authorize',
			grant : 'https://partner.api.beatsmusic.com/v1/oauth2/token'
		},
		...
		// API Base URL
		base : 'https://partner.api.beatsmusic.com/v1/api/',

		// Map GET requests
		get : {
			"audio" : 'users/:user_id/mymusic/tracks',
			"stream": 'tracks/:track_id/audio'
		},
...
hello.init({
	rdio : {
		...
		oauth : {
			version : 1,
			auth	: "https://www.rdio.com/oauth/authorize",
			request : 'http://api.rdio.com/oauth/request_token',
			token	: 'http://api.rdio.com/oauth/access_token'
		},
